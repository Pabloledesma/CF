trigger LeadConversion on Account (after insert) {
	// Falta agregar los nuevos campos del lead
    ...
    if(lead.size() > 0 && Trigger.new[0].RecordTypeId == rt[0].Id){
    	...
    	if(lead.size() > 0){
    		...
    		//Si hay codeudores asociados, crea cuenta(s) de tipo codeudor
            ...
            // Crea la solicitud de crédito y la radicación
            ...
            //Actualiza campos de los codeudores
            if(lstCodeudores.size() > 0){
                Integer i = 0;
    ...
    }// lead.size() > 0 && Trigger.new[0].RecordTypeId == rt[0].Id
}